...
public interface FrameDecoder<V extends Visitor>
{
	 ...
	 * version. If {@code autologin} is {@code true}, then the user-info part
	 * of the URI is used to automatically login to the server before returning
	 * the connection; if the user-info part is not present, it will throw
	 * {@link TPException}. See
	 * {@link Connection#makeConnection(FrameDecoder, URI)} for details.
	 * 
	 * @param serverUri
	 *            the connection parameters encoded as a {@link URI}
	 * @param autologin
	 *            if {@code true}, it will use user-info part of the
